/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as graphql from "@nestjs/graphql";
import { GraphQLError } from "graphql";
import { isRecordNotFoundError } from "../../prisma.util";
import { MetaQueryPayload } from "../../util/MetaQueryPayload";
import { Recommendation } from "./Recommendation";
import { RecommendationCountArgs } from "./RecommendationCountArgs";
import { RecommendationFindManyArgs } from "./RecommendationFindManyArgs";
import { RecommendationFindUniqueArgs } from "./RecommendationFindUniqueArgs";
import { CreateRecommendationArgs } from "./CreateRecommendationArgs";
import { UpdateRecommendationArgs } from "./UpdateRecommendationArgs";
import { DeleteRecommendationArgs } from "./DeleteRecommendationArgs";
import { Brief } from "../../brief/base/Brief";
import { Music } from "../../music/base/Music";
import { User } from "../../user/base/User";
import { RecommendationCreateInput } from "./RecommendationCreateInput";
import { RecommendationService } from "../recommendation.service";
@graphql.Resolver(() => Recommendation)
export class RecommendationResolverBase {
  constructor(protected readonly service: RecommendationService) {}

  async _recommendationsMeta(
    @graphql.Args() args: RecommendationCountArgs
  ): Promise<MetaQueryPayload> {
    const result = await this.service.count(args);
    return {
      count: result,
    };
  }

  @graphql.Query(() => [Recommendation])
  async recommendations(
    @graphql.Args() args: RecommendationFindManyArgs
  ): Promise<Recommendation[]> {
    return this.service.recommendations(args);
  }

  @graphql.Query(() => Recommendation, { nullable: true })
  async recommendation(
    @graphql.Args() args: RecommendationFindUniqueArgs
  ): Promise<Recommendation | null> {
    const result = await this.service.recommendation(args);
    if (result === null) {
      return null;
    }
    return result;
  }

  @graphql.Mutation(() => Recommendation)
  async createRecommendation(
    @graphql.Args() args: CreateRecommendationArgs
  ): Promise<Recommendation> {
    return await this.service.createRecommendation({
      ...args,
      data: {
        ...args.data,

        brief: args.data.brief
          ? {
              connect: args.data.brief,
            }
          : undefined,

        music: args.data.music
          ? {
              connect: args.data.music,
            }
          : undefined,

        user: args.data.user
          ? {
              connect: args.data.user,
            }
          : undefined,
      },
    });
  }

  @graphql.Mutation(() => Recommendation)
  async updateRecommendation(
    @graphql.Args() args: UpdateRecommendationArgs
  ): Promise<Recommendation | null> {
    try {
      return await this.service.updateRecommendation({
        ...args,
        data: {
          ...args.data,

          brief: args.data.brief
            ? {
                connect: args.data.brief,
              }
            : undefined,

          music: args.data.music
            ? {
                connect: args.data.music,
              }
            : undefined,

          user: args.data.user
            ? {
                connect: args.data.user,
              }
            : undefined,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new GraphQLError(
          `No resource was found for ${JSON.stringify(args.where)}`
        );
      }
      throw error;
    }
  }

  @graphql.Mutation(() => Recommendation)
  async deleteRecommendation(
    @graphql.Args() args: DeleteRecommendationArgs
  ): Promise<Recommendation | null> {
    try {
      return await this.service.deleteRecommendation(args);
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new GraphQLError(
          `No resource was found for ${JSON.stringify(args.where)}`
        );
      }
      throw error;
    }
  }

  @graphql.ResolveField(() => Brief, {
    nullable: true,
    name: "brief",
  })
  async getBrief(
    @graphql.Parent() parent: Recommendation
  ): Promise<Brief | null> {
    const result = await this.service.getBrief(parent.id);

    if (!result) {
      return null;
    }
    return result;
  }

  @graphql.ResolveField(() => Music, {
    nullable: true,
    name: "music",
  })
  async getMusic(
    @graphql.Parent() parent: Recommendation
  ): Promise<Music | null> {
    const result = await this.service.getMusic(parent.id);

    if (!result) {
      return null;
    }
    return result;
  }

  @graphql.ResolveField(() => User, {
    nullable: true,
    name: "user",
  })
  async getUser(
    @graphql.Parent() parent: Recommendation
  ): Promise<User | null> {
    const result = await this.service.getUser(parent.id);

    if (!result) {
      return null;
    }
    return result;
  }

  @graphql.Query(() => String)
  async FetchRecommendations(
    @graphql.Args()
    args: string
  ): Promise<string> {
    return this.service.FetchRecommendations(args);
  }

  @graphql.Query(() => [Recommendation])
  async FindMusicRecommendations(
    @graphql.Args()
    args: RecommendationCreateInput
  ): Promise<Recommendation[]> {
    return this.service.FindMusicRecommendations(args);
  }

  @graphql.Query(() => String)
  async SearchRecommendations(
    @graphql.Args()
    args: string
  ): Promise<string> {
    return this.service.SearchRecommendations(args);
  }
}
